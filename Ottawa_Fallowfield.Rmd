...
```{}
### Reverse geocode 
loczip <- lapply(seq(nrow(points_sample)), function(i){
google_reverse_geocode(location = c(points_sample$lat[i],points_sample$lon[i]), 
...
loczip_json <- toJSON(loczip)
parsed_locations <- fromJSON(loczip_json, simplifyVector = TRUE)

glimpse(parsed_locations)

### Get simple useable dataset
test <- as.data.frame(parsed_locations$plus_code)
test$origin <- test$compound_code
test$destination <- destination
test$compound_code <- NULL
test$global_code <- NULL
